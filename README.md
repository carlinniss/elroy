...
3.  **Setup Environment Variables:**
    ...
    TWITCH_BOT_USERNAME=your_bot_user
    TWITCH_BOT_OAUTH_TOKEN=oauth:your_bot_token_here
    TWITCH_OAUTH_TOKEN=oauth:your_broadcaster_token_here
    OVERLAY_CONTROL_SECRET=choose-a-long-random-secret
    GOOGLE_GENERATIVE_AI_API_KEY=your_google_ai_studio_key
    ELEVENLABS_API_KEY=your_elevenlabs_key
    ELEVENLABS_VOICE_ID=pNInz6obpgDQGcFmaJgB
    ```

    Do not prefix Twitch OAuth tokens with `NEXT_PUBLIC_`; chat sends and paid provider calls are handled by server routes.
...
2.  Set URL to `http://localhost:3000?controlKey=choose-a-long-random-secret`.
