...
3.  **Setup Environment Variables:**
    ...
    OVERLAY_CONTROL_SECRET=make_this_a_long_random_string
    GOOGLE_GENERATIVE_AI_API_KEY=your_google_ai_studio_key
    ELEVENLABS_API_KEY=your_elevenlabs_key
    ELEVENLABS_VOICE_ID=pNInz6obpgDQGcFmaJgB
    ```
    Only `NEXT_PUBLIC_TWITCH_CHANNEL` is sent to the browser. Keep OAuth and provider keys server-side.
...
1.  Start the development server:
    ...
    npm run dev
    ```
2.  Open `http://localhost:3000?controlKey=make_this_a_long_random_string` in your browser.
...
2.  Set URL to `http://localhost:3000?controlKey=make_this_a_long_random_string`.
