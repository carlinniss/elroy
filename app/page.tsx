...
function BongContent() {
  ...
  const setVoice = useCallback((enabled: boolean, user?: string) => {
    ...
    void sendChatMessage(user ? `@${user} voice ${enabled ? 'on' : 'off'}.` : `voice ${enabled ? 'on' : 'off'}.`).catch(console.error);
  }, [sendChatMessage]);
    ...
    client.on('message', (_c: string, t: tmi.ChatUserstate, m: string, s: boolean) => {
      ...
      if (m.toLowerCase() === '!voicestatus') {
        ...
        if (isBroadcaster || isModerator) {
          ...
          void sendChatMessage(`@${t.username} voice is ${state}.`).catch(console.error);
        }
        return;
      }
