...
function BongContent() {
  ...
  const authedFetch = useCallback((input: RequestInfo | URL, init: RequestInit = {}) => {
    ...
    if (controlKey) headers.set('x-overlay-control-secret', controlKey);
    return fetch(input, { ...init, headers });
  }, [controlKey]);

  const sayToChat = useCallback(async (message: string) => {
    const res = await authedFetch('/api/twitch/say', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message }),
    });
    if (!res.ok) throw new Error(`Twitch say failed (${res.status})`);
  }, [authedFetch]);
  ...
  const runDiagnostics = useCallback(async () => {
    try {
      const chat = await authedFetch('/api/chat', { method: 'POST', body: JSON.stringify({ prompt: 'ping' }) });
      const speech = await authedFetch('/api/speech', { method: 'POST', body: JSON.stringify({ text: 'ping' }) });
      const sound = await fetch('/sounds/bong.mp3');
      const quotaRes = await authedFetch('/api/quota');
      const qData = quotaRes.ok ? await quotaRes.json() : null;
  ...
  const speakNow = useCallback(async (text: string) => {
    try {
      const res = await authedFetch('/api/speech', { method: 'POST', body: JSON.stringify({ text }) });
  ...
  const processBongLogic = useCallback(async (input: string, user?: string, isQuota = false) => {
    try {
      if (isQuota) {
        const res = await authedFetch('/api/quota');
        if (!res.ok) throw new Error(`Quota failed (${res.status})`);
        const d = await res.json();
        await sayToChat(`${user ? `@${user} ` : ''}I got ${d.remaining.toLocaleString()} chars until ${d.resetDate}.`);
        return;
      }
      ...
      const res = await authedFetch('/api/chat', { method: 'POST', body: JSON.stringify({ prompt: fullPrompt }) });
      if (!res.ok) throw new Error(`Chat failed (${res.status})`);
      const data = await res.json();
      setLog(p => [{ text: data.text }, ...p].slice(0, 5));
      await sayToChat(user ? `@${user} ${data.text}` : data.text);
  ...
  }, [authedFetch, runDiagnostics, sayToChat, speak]);
  ...
  const toggleGong = useCallback(async (user?: string) => {
    const nextState = !gongEnabledRef.current;
    gongEnabledRef.current = nextState;
    setIsGongOn(nextState);
    await sayToChat(user ? `@${user} gong ${nextState ? 'on' : 'off'}.` : `gong ${nextState ? 'on' : 'off'}.`);
  }, [sayToChat]);

  const setVoice = useCallback(async (enabled: boolean, user?: string) => {
    voiceEnabledRef.current = enabled;
    setIsVoiceOn(enabled);
    await sayToChat(user ? `@${user} voice ${enabled ? 'on' : 'off'}.` : `voice ${enabled ? 'on' : 'off'}.`);
  }, [sayToChat]);
  ...
  const stopBot = useCallback(async (announceUser?: string) => {
    ...
    if (client) {
      try {
        if (announceUser) {
          await sayToChat(`@${announceUser} Elroy is off.`);
        }
  ...
  const startBot = async () => {
    ...
    client.on('message', (_c: string, t: tmi.ChatUserstate, m: string, s: boolean) => {
      ...
      if (m.toLowerCase() === '!gong') {
        ...
        if (isBroadcaster || isModerator) {
          return void toggleGong(t.username);
        }
        return;
      }
      ...
      if (m.toLowerCase() === '!voiceoff') {
        const isModerator = t.mod === true;
        if (isBroadcaster || isModerator) {
          return void setVoice(false, t.username);
        }
        return;
      }
      if (m.toLowerCase() === '!voiceon') {
        const isModerator = t.mod === true;
        if (isBroadcaster || isModerator) {
          return void setVoice(true, t.username);
        }
        return;
      }
      if (m.toLowerCase() === '!voicestatus') {
        ...
        if (isBroadcaster || isModerator) {
          ...
          void sayToChat(`@${t.username} voice is ${state}.`);
        }
        return;
      }
    ...
    setIsActive(true);
    void sayToChat('I AM ALIVE!').catch((e) => console.warn(e));
